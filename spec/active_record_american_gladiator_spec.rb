...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      # Changeable Start
      active_users = User.joins(:orders).select('users.*').distinct
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      orders = Order.joins(:items)
										.where("items.name = 'Football' OR items.name = 'Wrestling Ring'")
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      most_popular_items = Item.joins(:orders)
														 	 ...
														 	 .select('COUNT(order_items.item_id) AS count, items.*')
														 	 .order('count DESC') 
														 	 .limit(2)
      # Changeable Stop
